    ...
    <form method="post" id="form-editar-sede" class="row g-3 needs-validation-editar" enctype="multipart/form-data" novalidate>
            ...
            <select class="form-select" name="idempresa" id="empresa-editar" onchange="listarSedesEmpresa($(this).val(),2)" required>
                ...
                if (is_object($empresas[0])) {
                    foreach ($empresas as $empresa) {
                        if ($empresa->getId() == $sede->getIdempresa())
        ...
        <div class="col-md-6">
            ...
            <div class="form-check form-switch">
                <input class="form-check-input check-estado" name="estado" type="checkbox" id="flexSwitchCheckChecked" onchange="informacion()" <?php if (isset($_POST['estado'])) echo "checked";
                                                                                                                                                else if ($sede->getEstado() == 1) echo "checked"; ?> />
            </div>
        </div>
        <input type="hidden" name="foto" value="<?php if (isset($_POST['foto'])) echo $_POST['foto'];
                                                else echo $sede->getLogo(); ?>" />
